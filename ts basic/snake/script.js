...
class Snake {
    ...
    init(): void {
        ...
        this.tail = [{ x: 0, y: 0 }, { x: 10, y: 0 }, { x: 20, y: 0 },
        { x: 30, y: 0 }, { x: 40, y: 0 }];
        this.direction = Direction.Right;
        this.score = 0;
        this.gameOver = false;
    ...
    move(): void {
        ...
        switch (this.direction) {
            case Direction.Right:
                newHead.x = (head.x + 10 === canvas.width) ?
                    0 : head.x + this.speed;
                break;
            case Direction.Left:
                newHead.x = (head.x === 0) ?
                    (canvas.width - 10) : head.x - this.speed;
                break;
            case Direction.Down:
                newHead.y = (head.y + 10 === canvas.height) ?
                    0 : head.y + this.speed;
                break;
            case Direction.Up:
                newHead.y = (head.y === 0) ?
                    (canvas.height - 10) : head.y - this.speed;
                break;
        }
